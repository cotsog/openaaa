...
struct mempool_stats {      /* Mempool statistics. See @mp_stats(). */
	...
	uint chain_size[3];  /* Size of allocated chunks in small/big/unused chains */
};
...
void *
mp_alloc_internal(struct mempool *pool, uint size) _malloc;
 ...
 */
static inline void *
mp_alloc_fast(struct mempool *pool, uint size)
{
 ...
 */
static inline void *
mp_alloc_fast_noalign(struct mempool *pool, uint size)
{
...
/* For internal use only, do not call directly */
void *
mp_start_internal(struct mempool *pool, uint size) _malloc;
...
static inline void *
mp_start_fast(struct mempool *pool, uint size)
{
	uint avail = pool->state.free[0] & ~(CPU_STRUCT_ALIGN - 1);
	if (size <= avail) {
		pool->idx = 0;
...
static inline void *
mp_start_fast_noalign(struct mempool *pool, uint size)
{
	if (size <= pool->state.free[0]) {
		pool->idx = 0;
...
{
	return (byte *)pool->state.last[pool->idx] - 
	               pool->state.free[pool->idx];
}
 ...
 */
static inline void *
mp_grow(struct mempool *pool, uint size)
{
	return (size <= mp_avail(pool)) ? 
	        mp_ptr(pool) : mp_grow_internal(pool, size);
}
...
static inline void *
mp_spread(struct mempool *pool, void *p, uint size)
{
	return (((uint)((byte *)pool->state.last[pool->idx] - 
	       (byte *)p) >= size) ? p : mp_spread_internal(pool, p, size));
}
...
{
	void *p = mp_ptr(pool);
	pool->state.free[pool->idx] = (byte *)pool->state.last[pool->idx] - 
	                              (byte *)end;
	return p;
}
...
mp_open_fast(struct mempool *pool, void *ptr)
{
	...
	uint size = ((byte *)pool->state.last[pool->idx] - 
	            (byte *)ptr) - pool->state.free[pool->idx];
...
static inline void *
mp_realloc_fast(struct mempool *pool, void *ptr, uint size)
{
...
static inline void
mp_restore_fast(struct mempool *pool, struct mempool_state *state)
{
	if (pool->state.last[0] != state->last[0] || 
	    pool->state.last[1] != state->last[1])
...
char *
mp_strdup(struct mempool *, const char *) _malloc;

/* Makes a copy of a memory block on a mempool. */
void *
mp_memdup(struct mempool *, const void *, uint) _malloc;
/*x*
...
char *
mp_str_from_mem(struct mempool *p, const void *mem, uint len) _malloc;
