 ...
 **/
static inline byte *
utf8_get_repl(const byte *p, uint *uu, uint repl)
{
  uint u = *p++;
  if (u < 0x80)
    ;
 ...
 **/
static inline byte *
utf8_32_get_repl(const byte *p, uint *uu, uint repl)
{
  uint u = *p++;
  if (u < 0x80)
    ;
 ...
 **/
static inline byte *
utf8_32_get(const byte *p, uint *uu)
{
...
utf16_le_put(void *p, uint u)
{
  if (u < 0xd800 || (u < 0x10000 && u >= 0xe000))
  ...
  else if ((u -= 0x10000) < 0x100000)
    {
      ...
      put_u16_le((char *)p + 2, 0xdc00 | (u & 0x3ff));
      return (char *)p + 4;
    }
  else
    assert(0);
...
utf16_be_put(void *p, uint u)
{
	if (u < 0xd800 || (u < 0x10000 && u >= 0xe000)) {
		...
		put_u16_be((char *)p + 2, 0xdc00 | (u & 0x3ff));
		return (char *)p + 4;
	} else
		assert(0);
...
static inline void *
utf16_le_get_repl(const void *p, uint *uu, uint repl)
{
	uint u = get_u16_le(p), x, y;
	x = u - 0xd800;
	if (x < 0x800) {
    		if (x < 0x400 && (y = get_u16_le((char *)p + 2) - 0xdc00) < 0x400) {
			u = 0x10000 + (x << 10) + y;
			p = (char *)p + 2;
		} else u = repl;
	}
 ...
 **/
static inline void *
utf16_be_get_repl(const void *p, uint *uu, uint repl)
{
  uint u = get_u16_be(p), x, y;
  x = u - 0xd800;
  if (x < 0x800) {
    if (x < 0x400 && (y = get_u16_be((char *)p + 2) - 0xdc00) < 0x400)
      {
	u = 0x10000 + (x << 10) + y;
	p = (char *)p + 2;
      }
...
static inline void *
utf16_le_get(const void *p, uint *uu)
{
...
static inline void *
utf16_be_get(const void *p, uint *uu)
{
