...
parse_fastbuf(struct cf_context *cc, const char *name_fb, struct fastbuf *fb, uint depth)
{
  ...
  *p->line = 0;
  ...
  while (1)
  {
	...
	case 'c': op = islower(c[1]) == 'l' ? OP_CLEAR: OP_COPY; break;
	case 'a': switch (tolower(c[1])) {
		    case 'p': op = OP_APPEND; break;
		    case 'f': op = OP_AFTER; break;
		    default: op = OP_ALL;
		  }; break;
	case 'p': op = OP_PREPEND; break;
	case 'r': op = (c[1] && tolower(c[2]) == 'm') ? OP_REMOVE : OP_RESET; break;
...
load_file(struct cf_context *cc, const char *file)
{
  ...
  if (!fb) {
    sys_dbg("Cannot open configuration file %s: %m", file);
    return 1;
  }
