...
void *
gary_init(size_t elt_size, size_t num_elts, struct allocator *allocator)
{
  sys_dbg("GARY: Init to %zd elements", num_elts);
...
gary_realloc(struct gary_hdr *h, size_t n)
{
  ...
  if (n > 2*h->have_space)
    ...
    h->have_space *= 2;
  sys_dbg("GARY: Resize from %zd to %zd elements (need %zd)", old_size, h->have_space, n);
