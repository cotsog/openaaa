...
parse_session(request_rec *r, const char *file)
{
	...
	FILE *f = fopen(file, "r");
	r_info(r, "session file=%s parse", file);
	if (f == NULL || feof(f))
		return;
	...
	do {
		...
		*v = 0;
		...
		r_info(r, "%s=%s", n, v);
		aaa_attr_set(srv->aaa, n, (char *)v);

		if (!strcasecmp(n, "cn"))
			aaa_attr_set(srv->aaa, "user.name", (char *)v);

		if (!strcasecmp(n, "user.id"))
...
external_aaa(request_rec *r)
{
	...
	const char *authority = "orangebox-p2.aducid.com";
	...
	const char *handler = "/usr/local/bin/aducid";
	const char *cmd = stk_printf("%s -pr4 -a%s -i%s -k%s",
	                             handler, authority, id, key);

	r_info(r, "external authentication uri: %s", cmd);
...
post_read_request(request_rec *r)
{
	...
	r_info(r, "exporting keying material");
...
*/
	...
	r_info(r, "user.name: %s", aaa_attr_get(aaa, "user.name"));
	r_info(r, "user.id: %s", aaa_attr_get(aaa, "user.id"));

	if (!req->user.name)
		return HTTP_UNAUTHORIZED;

	r->user = apr_pstrdup(r->pool, req->user.name);
	r_info(r, "user.name: %s", r->user ? r->user : "nobody");
	return DECLINED;
}
...
access_checker(request_rec *r)
{
	...
	user->uuid = aaa_attr_get(aaa, "user.id");

	if (!user->uuid)
		return OK;
	
	r->user = user->name ? apr_pstrdup(r->pool, user->name) : user->uuid;
	apr_table_t *t = r->subprocess_env;

	apr_table_add(t, "REMOTE_USER", user->name);
	const char *id   = aaa_attr_get(aaa, "sess.id");
	const char *sec  = aaa_attr_get(aaa, "sess.sec");
        const char *key  = aaa_attr_get(aaa, "sess.key");
	const char *akey = aaa_attr_get(aaa, "aducid.auth.key");
	const char *aid  = aaa_attr_get(aaa, "aducid.auth.id");

	/* TODO: Fixup AJP Binding */
        apr_table_setn(t, "AJP_TLS_BINDING_KEY",  key);
        apr_table_setn(t, "AJP_TLS_BINDING_ID",  sec);
	apr_table_setn(t, "AJP_ADUCID_AUTH_KEY",  akey);
	apr_table_setn(t, "AJP_ADUCID_AUTH_ID",  aid);
	apr_table_setn(t, "AJP_AAA_SESSION_ID",  id);

	r_info(r, "aaa.session.id: %s", id);
	r_info(r, "tls_binding_key: %s", key);
	r_info(r, "tls_binding_id: %s", sec);
	r_info(r, "aducid.auth.key: %s", akey);
	r_info(r, "aducid.auth.id: %s", aid);
