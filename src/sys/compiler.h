...
/*
 * The le32 and be32 represent 32-bit integer types with different endianness.
 * However, the C language does not allow to specify that variables of these 
 * types should not be mixed. The bitwise attribute is used to mark these 
 * types as restricted, so Sparse will give a warning if variables of these 
 * types or other integer variables are mixed.
 */
